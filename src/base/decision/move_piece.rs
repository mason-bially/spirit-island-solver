    ...
    fn apply_effect(&self, game: &mut GameState) -> Result<(), StepFailure> {
        ...
        let total_source_count: usize;
        {
            ...
            if total_source_count == 0 {
                game.log_effect(format_args!("push {} {} from {} (but no sources!).", self.count, piece_kind_vec_to_string(&self.kinds), self.land_index));
                return Ok(());
            }
        }

        if self.may {
            game.log_decision(format_args!("may push {} {} from {}.", self.count, piece_kind_vec_to_string(&self.kinds), self.land_index));
        }
        else {
            game.log_decision(format_args!("push {} {} from {}.", self.count, piece_kind_vec_to_string(&self.kinds), self.land_index));
        }
